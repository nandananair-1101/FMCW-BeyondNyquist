...
- **`RadarAnalysis_RCS_CFAR.m`**  
  Provides a comprehensive radar analysis suite covering system diagnostics, 1D/2D FFT profiles, multi-target resolution matrices, Radar Cross Section (RCS) scaling, target variations with noise addition (same range/different Doppler and same Doppler/different range), and Cell-Averaging CFAR detection.
...
- **`multiTarget_fft_iff.m`**  
  Resolves multiple targets beyond standard limits by combining 2D Fast Fourier Transform (FFT) rough binning with iterative IFF refinement and target subtraction.
